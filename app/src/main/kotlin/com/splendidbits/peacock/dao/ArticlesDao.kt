...
interface ArticlesDao {
    ...
    @Transaction
    @Query("select * from batches order by datetime(updated) desc limit 1")
    fun getLatestBatch(): LiveData<Batch?>

    @Transaction
    @Query("select * from items inner join batch_item on batch_item.batchId = :batchId and items.itemId = batch_item.itemId")
    fun getItems(batchId: String?): LiveData<List<Item>>

    @Transaction
    @Query("select * from assets inner join item_asset on item_asset.itemId = :itemId and assets.assetId = item_asset.assetId")
